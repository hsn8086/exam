...
seg_tree = LazySegmentTree(
    ...
    update_func=lambda val, tag, l, r: (r - l + 1) - val if tag else val,
...
for _ in range(m):
    ...
    if cmd == 0:
        seg_tree.range_update(next(data), next(data), 1)
    elif cmd == 1:
        ans.append(seg_tree.range_query(next(data), next(data)))
sys.stdout.write("\n".join(map(_str, ans)))
